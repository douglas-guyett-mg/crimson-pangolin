...
5. CREATE PLAN PATH:
   ...
   - Go to Step 7 (Activate Hobgoblins)
   ↓
6. EVALUATE EXISTING PLAN PATH:
   ...
   - Go to Step 7 (Activate Hobgoblins)
   ↓
7. ACTIVATE HOBGOBLINS:
   - Use plan to determine which hobgoblins to activate
   - Set urgency level
   - Set expected flow
   - Activate hobgoblins
   ↓
...
## Integration with Hobgoblin Activation

Router uses the plan to determine which hobgoblins to activate:
...
Analyze plan steps
  ↓
Determine required hobgoblins:
  - If plan has research steps: activate Context Assembler
  - If plan has constraint checks: activate Constraint Checker
  ...
  ↓
Activate determined hobgoblins
```
...
- `documentation/turn/hobgoblins/router.md` - Router hobgoblin
