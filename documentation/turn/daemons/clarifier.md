...
The Clarifier handles ambiguous or incomplete user inputs. When Router detects that clarification is needed, Clarifier identifies what's unclear and works with Responder to generate targeted follow-up questions.

## Responsibilities

1. **Detect Ambiguity**: Identify what's unclear in the input
2. **Analyze Missing Information**: Determine what context is needed
3. **Prioritize Questions**: Decide which questions are most important
4. **Provide to Responder**: Pass clarification needs to Responder daemon
5. **End Turn**: Responder sends clarification request and ends the turn
...
## Interaction with Responder

```
Router: "Input is ambiguous"
  ↓
Router activates Clarifier
  ↓
Clarifier analyzes ambiguity
  ↓
Clarifier passes to Responder: "Generate clarifying questions"
  ↓
Responder generates questions
  ↓
Responder sends to user
  ↓
Turn ends, waiting for user response
```
...
**Clarifier prevents wasted work.** By asking clarifying questions upfront, Clarifier ensures that subsequent daemons (Plan Generator, Executor, etc.) have clear direction and don't waste time on wrong assumptions.
