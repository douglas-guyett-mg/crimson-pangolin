# Router Daemon - Test Conditions

## Overview

This document specifies comprehensive test conditions for the Router daemon. Router is the entry point for every turn and makes critical decisions about urgency assessment and daemon activation.
...
**Verification**:
- Urgency level = HIGH
- Confidence calculated as: (matching_patterns / total_patterns) * pattern_weight, minimum 3 HIGH-urgency patterns detected, score normalized 0.0-1.0
...
- Confidence calculated as: (matching_patterns / total_patterns) * pattern_weight, minimum 2 MEDIUM-urgency patterns detected, score normalized 0.0-1.0
...
**Verification**:
- Activated daemons = [Responder]
- No other daemons activated
- Exactly 1 daemon activated, no intermediate daemons, response path: Input → Router → Responder → Output, total hops = 3
...
**Verification**:
- Activated daemons includes Clarifier
- Clarifier is activated before other daemons
...
**Verification**:
- Activated daemons includes all required daemons
- Hobgoblins activated in correct order
- All daemons receive necessary context
...
### Test R.2.5: Constraint-Sensitive Query - Executor Constraint Checking
...
**Expected**: Router activates Executor which checks constraints
**Verification**:
- Executor is activated
- Executor checks constraints before action
- Constraints are verified before execution
...
## Integration Tests: Daemon Coordination

### Test R.3.1: Parallel Daemon Activation
**Condition**: Router activates multiple daemons that can run in parallel
**Expected**: Daemons run concurrently without conflicts
**Verification**:
- All daemons complete successfully
- No race conditions detected
- Results are consistent
- Execution time < sequential baseline * 0.6 (40% improvement minimum), measured with all daemons at 100ms baseline

### Test R.3.2: Sequential Daemon Activation
**Condition**: Router activates daemons with dependencies
**Expected**: Daemons execute in correct order
**Verification**:
- Dependencies respected
- Each daemon receives output from previous
- No deadlocks occur

### Test R.3.3: Context Passing
**Condition**: Router passes context to activated daemons
**Expected**: All daemons receive necessary context
**Verification**:
- Context includes urgency level
- Context includes turn goals
- Context includes relevant history
- All daemons can access context
...
**Verification**:
- Urgency level matches highest-severity signal (safety/security always wins), reasoning cites specific conflicting signals and resolution rule applied
- Reasoning explains conflict resolution
- User safety prioritized

### Test R.4.2: Ambiguous Daemon Activation
**Condition**: Query could activate multiple different daemon patterns
**Expected**: Router selects most appropriate pattern
**Verification**:
- Selected pattern has highest confidence score among alternatives (difference >= 0.15), or if scores within 0.15 then pattern with fewest daemons chosen
...
- Explicit user feedback score increases by >= 0.1 points per 100 turns, implicit satisfaction (task completion rate) increases by >= 2%, measured over 7-day rolling window
...
**Verification**:
- Decision time < 100ms
- All operations use async/await or callbacks, thread pool never shows wait states > 1ms, CPU time = wall time ± 10ms indicating pure CPU work
...
2. ✓ Daemon activation is appropriate
3. ✓ Context is correctly passed to daemons
